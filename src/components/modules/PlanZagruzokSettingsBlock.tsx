...
import {UserProfile, AppSettings, PlanZagruzokTab} from '../../types'
...
export default function PlanZagruzokSettingsBlock({ user }: Props) {
  ...
  const handleAddTab = () => {
    ...
    dbService.saveSettings({
      ...settings,
      planZagruzokTabs: [...(settings.planZagruzokTabs || []), newTab]
    }, user.name, user.role);
    toast('Вкладка добавлена', 'success');
  };

  const handleUpdateTab = (id: string, field: keyof PlanZagruzokTab, value: string) => {
    if (!settings) return;
    const tabs = settings.planZagruzokTabs || [];
    const updated = tabs.map(t => t.id === id ? { ...t, [field]: value } : t);
    dbService.saveSettings({
      ...settings,
      planZagruzokTabs: updated
    }, user.name, user.role);
  };
  ...
  const handleDeleteTab = (id: string) => {
    ...
    dbService.saveSettings({
      ...settings,
      planZagruzokTabs: tabs.filter(t => t.id !== id)
    }, user.name, user.role);
      ...
      <div className="flex items-center justify-between border-b border-white/40 pb-4">
        <div>
          <span className="bg-indigo-600/10 text-indigo-700 border border-indigo-500/10 font-bold text-[9px] px-2.5 py-0.5 rounded-full uppercase font-mono tracking-wider">
            Loading Plans Configuration
          </span>
          <h2 className="text-sm font-bold tracking-tight text-slate-900 mt-2 flex items-center gap-1.5">
            План загрузок (вкладки)
          </h2>
          <p className="text-[10px] text-slate-500 font-medium mt-1 leading-relaxed">
            Управление листами Google Sheets для планирования загрузок.
          </p>
        </div>
        <button 
      ...
      <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Например, План загрузок 1"
                />
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    ...
                    className="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose border border-white/45 shadow-xs rounded-xl cursor-pointer transition-all active:scale-95 flex items-center justify-center shrink-0"
        ...
        {tabs.length === 0 && (
          <div className="text-center py-10 text-slate-500 text-xs font-medium bg-white/40 backdrop-blur-md rounded-[1.5rem] border border-white/45 shadow-inner flex flex-col items-center gap-3">
            ...
            <span className="font-semibold text-slate-500 tracking-wider text-[10px] font-mono">Нет добавленных подвкладок</span>
          </div>
        )}
      </div>
    </div>
