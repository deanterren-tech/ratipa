    ...
    >
      ...
      <div className="px-3.5 py-2.5 border-b border-slate-100/60 bg-slate-50/30 flex items-start justify-between gap-3">
        <div className="space-y-0.5 min-w-0 flex-1">
          ...
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wide font-sans truncate" title={brandsText}>
            {brandModel ? (
              <span>Тягач: <span className="text-slate-700">{brandModel}</span>{trailerMake ? <> / Прицеп: <span className="text-slate-700">{trailerMake}</span></> : ''}</span>
            ) : rec.brandsLat ? (
      ...
      <div className="px-3.5 py-2 flex-1 flex flex-col justify-end bg-slate-50/5">
        <div className="bg-[#3765F6]/5 border border-[#3765F6]/10 rounded-xl p-2 font-mono text-[9.5px] text-slate-600 leading-tight relative group">
          ...
          <div className="select-all font-bold text-slate-800 leading-none mb-0.5">{rec.vehicleNumbers}</div>
          <div className="select-all truncate">Марки: {brandsText || '—'}</div>
          <div className="select-all truncate">Водитель: {formatDriverShortName(rec.driverNameRu || (rec as any).driverName)}</div>
          <div className="select-all">Д.Р.: {rec.birthDate || '—'}</div>
          <div className="select-all">Паспорт: {rec.passportNumber || '—'}</div>
          <div className="select-all truncate">Л.Н.: {rec.personalId || '—'}</div>
...
export default function VehicleDriverDataModule({ user }: VehicleDriverDataModuleProps) {
  ...
  const hasInitializedVerification = useRef(false);
  ...
  useEffect(() => {
    ...
    if (pendingVerifications.length > 0 && !hasInitializedVerification.current && !currentVerification) {
      hasInitializedVerification.current = true;
...
${brandsText ? `Марки: ${brandsText}\n` : ''}Водитель: ${driverNameText}
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-slate-200/40 rounded-3xl p-6 shadow-xs space-y-6">
            ...
            {filteredRecords.length === 0 ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto pr-1 custom-scrollbar">
