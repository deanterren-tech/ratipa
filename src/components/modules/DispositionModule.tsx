...
export default function DispositionModule({ user }: { user: any }) {
  ...
  // GPS-блокнот: позиция и размер (drag + resize), как в эталоне
  ...
  return (
    <div className="fixed top-16 inset-x-0 bottom-0 z-40 bg-slate-100 overflow-hidden">
      ...
      {/* overlay при drag/resize GPS (чтобы iframe не перехватывал события) */}
      ...
      {collapsed ? (
        ...
        >
          <ChevronDown className="h-4 w-4" /> Диспозиция
        </button>
      ) : (
              ...
              <div className="flex items-center bg-slate-100 p-1 rounded-xl gap-1 shrink-0" onMouseDown={(e) => e.stopPropagation()}>
                <button
                  ...
                  className={`px-3 py-1 text-[10px] font-bold tracking-tight rounded-lg transition-all duration-150 cursor-pointer ${
                    gpsTab === "beltranssputnik" ? "bg-white text-slate-900 shadow-xs border border-slate-200/30" : "text-slate-500 hover:text-slate-800 hover:bg-white/30"
                  }`}
                >
                  Белтранс
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-[10px] font-bold tracking-tight rounded-lg transition-all duration-150 cursor-pointer ${
                    gpsTab === "wialon" ? "bg-white text-slate-900 shadow-xs border border-slate-200/30" : "text-slate-500 hover:text-slate-800 hover:bg-white/30"
                  }`}
                >
                  Wialon
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-[10px] font-bold tracking-tight rounded-lg transition-all duration-150 cursor-pointer ${
                    gpsTab === "era_glonass" ? "bg-white text-slate-900 shadow-xs border border-slate-200/30" : "text-slate-500 hover:text-slate-800 hover:bg-white/30"
                  }`}
                >
                  ГЛОНАСС
                </button>
