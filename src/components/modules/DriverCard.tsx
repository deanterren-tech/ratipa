...
export default function DriverCard({ driverId, driverName, onClose, onOpenCoupling }: DriverCardProps) {
  ...
  useEffect(() => {
    ...
    const u1 = dbService.getDrivers((list: any[]) => {
      const found = (list || []).find((d) => d.id === driverId || (d.shortNameRu || d.name || '') === driverName);
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <button onClick={() => onOpenCoupling(coupling.carNumber || coupling.vehicleNumbers || '')}
              ...
              <div className="flex-1">
                ...
                <div className="text-sm font-semibold text-slate-800 font-mono">
                  {coupling.carNumber || coupling.vehicleNumbers}
                  {coupling.trailerNumber ? ` + ${coupling.trailerNumber}` : ''}
                </div>
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <Field label="ФИО (рус)" value={driver?.name || driverName} />
              <Field label="ИНН" value={driver?.personalId || driver?.inn || '—'} />
              <Field label="Паспорт" value={driver?.passportNumber || '—'} />
              <Field label="Телефон" value={driver?.phone || driver?.phones?.[0]?.number || '—'} />
            </div>
          </div>
