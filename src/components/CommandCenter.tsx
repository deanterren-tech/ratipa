...
import { Search, MapPin, FileText, DollarSign, User, Truck, X } from 'lucide-react';
...
export default function CommandCenter({ user, isOpen, onClose, onNavigate }: CommandCenterProps) {
  ...
  useEffect(() => {
    ...
    const searchData = () => {
       const bazaRef = ref(database, 'baza_cars');
       const unsub = onValue(bazaRef, (snapshot) => {
          const data = snapshot.val() || {};
          const found = Object.keys(data).map(k => ({id: k, ...data[k]})).filter(t => 
